...
// Copyright (c) 2014 The Germanycoin developers
...
uint256 hashGenesisBlock("0xae2782c02092434d8a25aae1162fd63aa73fe0556c9f292e4408efaf81769a0e");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Germanycoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Germanycoin Signed Message:\n";
...
{
    // Germanycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    // Subsidy is cut in half every 8*30*24*30 blocks, which will occur approximately every 8 months
    //nSubsidy >>= (nHeight / 8*30*24*30); // Germanycoin blocks in 8 months
	int64 ger_sub_bin;
	int64 nSubsidy;
	if (nHeight==1) return 21000000 * COIN + nFees; // block 1 contains the 21M premine

    // 100 Coin Payout at Reuniion Day, October, 3rd
    if (nHeight>=280980 && nHeight<= 282420 || nHeight>=806580 && nHeight<= 808020 || nHeight>=1333620 && nHeight<= 1335060 || nHeight>=1859220 && nHeight<= 1860660 || nHeight>=2384820 && nHeight<= 2386260) return 100 * COIN + nFees;

    if (nHeight <= 1382400) {
        ger_sub_bin = (nHeight/(1440*40));
        nSubsidy = 25-ger_sub_bin; // Germanycoin subsidy decreases gradually, without sudden halvings
        nSubsidy *= COIN;
    }
    else { //payout stays equal at 1 GER, from block 1382400 on untill all coins are mined
        nSubsidy = COIN;
    }
...
static const int64 nTargetSpacing = 2 * 60; // Germanycoin: 2 minutes
        ...
        for (unsigned int i = 1; BlockReading && BlockReading->nHeight > 0; i++) {
            ...
            if (PastBlocksMax > 0 && i > PastBlocksMax) { break; }
                ...
                if (LatestBlockTime < BlockReading->GetBlockTime()) {
                    if (BlockReading->nHeight > 11500) // HARD Fork block number
...
{
        static const int64 BlocksTargetSpacing = 2 * 60; // 2 minutes for Germanycoin
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Germanycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
{
    // Germanycoin: temporarily disable v2 block lockin until we are ready for v2 transition
    return false;
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xae2782c02092434d8a25aae1162fd63aa73fe0556c9f292e4408efaf81769a0e");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Putin took over the Krim";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
    ...
    txNew.vout[0].nValue = 100 * COIN;
        ...
        block.nVersion = 1;
        block.nTime    = 1395398354; //germanycoin 1395398354;
        block.nBits    = 0x1e0fffff;
        block.nNonce   = 388020539; //2084808754; //2084524493;

        if (fTestNet)
        {
            block.nTime    = 1395248274;
            block.nNonce   = 385700993;
        }
    ...
    assert(block.hashMerkleRoot == uint256("0x519cceb021f2dd435858a0049962ab93ff0857dabe2b556616818e0d32823726"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0x14, 0x92, 0x00 }; // Germanycoin
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("GermanycoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("GermanycoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static GermanycoinMiner(CWallet *pwallet)
{
    printf("GermanycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("germanycoin-miner");
    ...
    try { loop {
        ...
        printf("Running GermanycoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("GermanycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&GermanycoinMiner, pwallet));
}
